...
from app.models import (
    ...
    Transaction, TransactionType, Notification, Dispute, DisputeStatus
)
...
def get_products(
    ...
    result = []
    for p, seller in results:
        # Получить первое фото из массива
        first_image = None
        if p.images:
            try:
    ...
    if is_cacheable:
        try:
            cache.set(cache_key, json.dumps(result, ensure_ascii=False, default=str), ex=60)
...
def create_order(
    ...
    platform_fee = round(total_price * 5 / 100)  # 5% комиссия
...
def cancel_order(
    ...
    if order.status != OrderStatus.pending:
        raise HTTPException(400, "Отменить можно только новый заказ (pending)")
