...
class User(Base):
    ...
    def is_pro_active(self) -> bool:
        """Действует ли PRO прямо сейчас — по дате, а не по флагу.

        `is_pro` — это намерение («подписка была оформлена»), а не состояние.
        Снять флаг должна задача `check_expired_pro_subscriptions`, и она теперь
        действительно запускается: воркер и beat подняты в контурах развёртывания
        (docker-compose.prod.yml, render.yaml, ecosystem.config.cjs).

        Но читать права из флага всё равно нельзя, и вот почему. Пока воркер не
        поднимался, задача не исполнялась никогда — и истёкшая подписка давала
        безлимит откликов и 0% комиссии бессрочно. Проверка по дате делает это
        невозможным независимо от того, успел ли пройти плановый запуск: между
        истечением срока и ближайшим запуском задачи в 9:00 проходит до суток,
        и всё это время флаг остаётся `True`.

        Условие «`is_pro` и `pro_until` в будущем» заодно закрывает случай,
        когда подписка выдана без срока (`pro_until IS NULL`) — такая не
        действует, потому что подтвердить её действие нечем.

        ВАЖНО: всё, что считает права (комиссия, лимит откликов, метки PRO),
        должно использовать это свойство, а не колонку напрямую.
        """
...
class StoredFile(Base):
    ...
    # Кто загрузил. Нужен, чтобы владелец мог удалить свой файл, а чужой — нет.
    # NULL у записей, сделанных до появления колонки: владелец неизвестен,
    # поэтому удалять их через API нельзя (см. DELETE /files/{id}).
    owner_id = Column(Integer, nullable=True, index=True)

    # Приватный файл отдаётся только по ссылке с подписью.
    #
    # `GET /files/{id}` был публичным, а `id` — последовательным целым, то есть
    # любая загруженная картинка читалась перебором от 1. Пока через загрузку
    # проходили только аватар, фото задания и портфолио (публичные по смыслу),
    # это было безобидно. Но вложения в чате — личная переписка, и как только
    # они переехали с base64 на `/files/`, публичный эндпоинт стал бы утечкой.
    #
    # Поэтому у файла есть признак приватности, а доступ к приватному даётся
    # подписью в query-строке: подпись выдаётся только участникам сделки вместе
    # с сообщением. `default=False` — старые записи остаются публичными и
    # ничего не ломается.
