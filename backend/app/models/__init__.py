...
class User(Base):
    ...
    def is_pro_active(self) -> bool:
        ...
        Снять флаг должна была celery-задача `check_expired_pro_subscriptions`,
        но `celery` нет в зависимостях и ни один контур развёртывания не
        поднимает воркер — задача не исполняется никогда. Пока права читались
        из флага, истёкшая подписка продолжала давать безлимит откликов и 0%
        комиссии бессрочно.

        Проверка по дате делает это невозможным: флаг может оставаться `True`
        сколько угодно, но права он больше не даёт. Условие «`is_pro` и
        `pro_until` в будущем» заодно закрывает случай, когда подписка
        выдана без срока (`pro_until IS NULL`) — такая не действует, потому
        что подтвердить её действие нечем.
