...
уровне импортирует `celery`, поэтому его нельзя ни запустить, ни протестировать,
не поднимая celery. А это функция, которая удаляет данные: её нужно уметь
проверить отдельно от брокера.

(На момент выноса `celery` не было и в `backend/requirements.txt`, и воркер не
поднимал ни один контур развёртывания — теперь и то, и другое исправлено, но
разделение осталось: проверка того, что удаляет данные, не должна зависеть от
наличия брокера.)
