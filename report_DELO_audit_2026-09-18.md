...
**7. Истёкшая PRO-подписка сохраняет все привилегии — денег не приносит, но
раздаёт бесплатно.** Подтверждено на живом стенде.
...
**8. ЮKassa: баланс зачисляется без подтверждения реальной оплаты.**
Денежный дефект, воспроизведение — `tests/security/repro_yookassa_unpaid_credit.py`.

`POST /payments/confirm?provider=yookassa` берёт `payment_id` из query-строки
(то есть от клиента) и передаёт его в `payments.get_payment_status()`, который
...
опирается на единственный внешний вызов: если провайдер отвечает
`paid: true`, деньги зачисляются.

`confirmation_url` не сохраняется и при подтверждении не проверяется — значит,
пользователь не обязан ни разу открыть страницу оплаты. Запись `PaymentRecord`
при этом ищется по тому же значению, которое прислал клиент, и найденная
запись используется как достаточное подтверждение.
...
`IS_PRODUCTION` этот путь не закрывает: он отключает только демо-пополнение
`POST /wallet/deposit`.

Что нужно: не доверять клиентскому `payment_id` как источнику истины —
сверять, что подтверждаемый платёж действительно порождён нашим запросом
(проверять `confirmation_url`/статус в момент создания и сохранять результат),
и сличать сумму от провайдера с `record.amount`, как это уже сделано в
вебхуке ЮMoney. Вебхука для ЮKassa не существует вовсе — маршруты платежей
сейчас: `/payments/create`, `/payments/confirm`, `/payments/status`,
