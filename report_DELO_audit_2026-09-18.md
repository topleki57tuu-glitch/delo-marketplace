...
> плюс по ходу проверки найдены и закрыты ещё четыре дефекта (WebSocket-broadcast,
> CSV-экспорт, подтверждение платежа ЮKassa без доказательства оплаты, бессрочные
> права истёкшей PRO-подписки). Пароль `demo123` вычищен из всех 22 файлов, включая
> документацию. Полный прогон тестов — **125 проверок, 0 падений**.
...
| `tests/security/repro_yookassa_unpaid_credit.py` | 6 проверок, 0 падений (новый) | — |
| `tests/security/check_pro_expiry.py` | 9 OK (новый) | — |

`tests/security/repro_yookassa_unpaid_credit.py` — регресс на дефекты
подтверждения платежей (п. 7). Три сценария из шести проверок: голый
`payment_id` (400), подделанный `confirmation_url` (403), верный `confirmation_url`
(200, зачисление) — плюс ветка ЮMoney: чужой платёж (403), свой (200),
старая запись без URL (409). Код возврата: 0 — защита работает, 1 — дыра жива,
2 — фикс переусердствовал и блокирует штатный путь. Смысл последнего в том,
что защита, которая просто «всё запрещает», тоже поломка — её нужно ловить.

**Итого 125 проверок на SQLite и 87 на PostgreSQL, 0 падений** при `CSRF_ENABLED=1`.
...
### Найдено при проверке после починки CI

**7. ЮKassa: баланс зачисляется без подтверждения реальной оплаты.**
ИСПРАВЛЕНО (коммит `e53f0fe`). Денежный дефект, воспроизведение —
`tests/security/repro_yookassa_unpaid_credit.py`.

`POST /payments/confirm?provider=yookassa` брал `payment_id` из query-строки
(то есть от клиента) и передавал его в `payments.get_payment_status()`, который
делает `GET /payments/{payment_id}` у провайдера. Ветка ЮMoney здесь надёжна
(`record.user_id != user_id` → 403, сумма сверяется), а ветка ЮKassa
опиралась на единственный внешний вызов: если провайдер отвечал
`paid: true`, деньги зачислялись.

Запись `PaymentRecord` искалась по тому же значению, которое прислал
клиент, — своя запись и её же аргумент служили друг другу доказательством.
`confirmation_url` не сохранялся и при подтверждении не проверялся нигде,
поэтому пользователь не обязан был ни разу открыть страницу оплаты.

Воспроизведение (провайдер сообщает `paid: true`, денег никто не платил):

```
баланс до:    0
ответ:        http=200 {"status":"succeeded","credited":true,"new_balance":50000}
баланс после: 50000
```

`IS_PRODUCTION` этот путь не закрывал: он отключает только демо-пополнение
`POST /wallet/deposit`.

**Как исправлено.** `confirmation_url` теперь сохраняется в `PaymentRecord`
при создании платежа и сверяется при подтверждении
(`app/api/payments.py::_verify_local_record`). URL формирует провайдер, к
клиенту он попадает только через нас — значит, его предъявление и есть
доказательство, что подтверждается именно созданный нами платёж. Проверка
локальная (к провайдеру не ходит) и выполняется до обращения к нему. Владелец
платежа проверяется в обеих ветках.

Записи без сохранённого URL (созданные до появления колонки) отклоняются с
409: «не знаю, что это за платёж» должно означать отказ, а не пропуск
проверки. Такие платежи закрываются вебхуком, где источник истины — `label`.

Отдельно проверено, что фикс не сломал штатный путь:

| Сценарий | Результат |
|---|---|
| голый `payment_id` без `confirmation_url` | 400, зачислено 0 |
| выдуманный `confirmation_url` | 403, зачислено 0 |
| верный `confirmation_url` | 200, зачислено 50000 |
| ЮMoney: чужой платёж из-под своего аккаунта | 403 |
| ЮMoney: своя запись с верным URL | 200, идемпотентно |
| ЮMoney: старая запись без URL | 409 |

Вебхука для ЮKassa по-прежнему нет — маршруты платежей:
`/payments/create`, `/payments/confirm`, `/payments/status`,
`/payments/webhook/yoomoney`.

**8. Истёкшая PRO-подписка сохраняет все привилегии.**
ИСПРАВЛЕНО. Регресс — `tests/security/check_pro_expiry.py` (9 проверок).
Подтверждено на живом стенде.

Единственное место, которое снимает флаг `is_pro`, — celery-задача
`app/tasks/cleanup.py:135 check_expired_pro_subscriptions`, поставленная в beat
на 9:00 (`app/core/celery_app.py:72`). Но **`celery` отсутствует в
`backend/requirements.txt`**, а воркера и beat нет ни в `docker-compose.prod.yml`,
ни в `ecosystem.config.cjs`, ни в `render.yaml`. Задача не исполняется никогда.

Все потребители смотрели только на булев флаг и никогда — на `pro_until`:

```python
# backend/app/api/responses.py:42
if not specialist.is_pro:                      # pro_until не смотрится
    if (specialist.response_credits or 0) <= 0:
        raise HTTPException(402, "Отклики закончились...")

# backend/app/api/tasks.py:313 (то же в products.py:236, disputes.py:337)
fee_percent = 0 if is_pro else 5               # комиссия 0% навсегда
```

Проверено на стенде: `igor@delo.ru`, `is_pro=True`,
`pro_until=2026-09-17T10:28:24` (вчера), `response_credits=0`:

```
POST /tasks/{id}/responses  ->  http=200
   {"message":"Отклик отправлен","credits_left":null}
```

То есть бесплатный отклик и 0% комиссии при истёкшей подписке. `users.py:140`
отдаёт `pro_until` наружу, но ни одна проверка прав его не читает.

**Как исправлено.** Взят вариант «проверять срок в момент чтения», а не
«поднимать Celery»: окно до 24 часов исчезает полностью, и не нужно добавлять
воркер + beat в три контура развёртывания, которые его сегодня не поднимают.

Ключевая мысль: `is_pro` — это **намерение** («подписка оформлялась»), а
`pro_until` — **состояние**. Права должны считаться по состоянию:

```python
# backend/app/models/__init__.py — User.is_pro_active
if not self.is_pro or self.pro_until is None:
    return False
return self.pro_until > datetime.utcnow()
```

Условие намеренно строгое: `pro_until IS NULL` → **не действует** (fail-closed).
Подписка без срока не подтверждается ничем, поэтому прав не даёт. Это
сознательно жёстче, чем «нет срока — значит навсегда».

Заменено во всех местах, где считаются права или рисуется метка PRO — 16
обращений к `is_pro_active` в семи файлах:

| Файл | Что изменено |
|---|---|
| `responses.py` | безлимит откликов, текст уведомления, `credits_left`, `specialist_pro` |
| `tasks.py` | комиссия при завершении сделки |
| `products.py` | комиссия по заказу товара |
| `disputes.py` | комиссия при разрешении спора |
| `users.py` | `/users/me`, публичный профиль, список специалистов |
| `admin.py` | сводка, счётчик подписок, фильтр, строка выдачи (+ `pro_until` в ответ) |
| `payments.py` | ответ на покупку пакета |

Осталось ровно два чтения колонки `is_pro` — тело самого свойства и присваивание
`user.is_pro = False` внутри так и не запускаемой celery-задачи. В админке
фильтр по SQL написан явно (`is_pro == True AND pro_until IS NOT NULL AND
pro_until > now()`), потому что Python-свойство в SQL не транслируется:
иначе фильтр «PRO» перечислял бы и тех, у кого подписка давно истекла.

Регресс `check_pro_expiry.py` устроен так, что откат фикса его ломает. Проверено
контрольным откатом двух вызовов на колонку — падают ровно три проверки, и все
по делу:

```
FAIL истёкшая PRO не даёт бесплатный отклик | http=200   (должно быть 402)
FAIL флаг без срока не даёт бесплатный отклик | http=200
FAIL истёкшая PRO платит стандартную комиссию | комиссия=0   (должно быть 500)
=== ИТОГ: passed=6 failed=3 ===
```
...
1. Отклики при исчерпанных кредитах
  OK  истёкшая PRO не даёт бесплатный отклик | http=402
  OK  флаг без срока не даёт бесплатный отклик | http=402
  OK  действующая PRO отклик пропускает | http=200
2. Комиссия при завершении сделки (бюджет 10000)
  OK  истёкшая PRO платит стандартную комиссию | комиссия=500
  OK  действующая PRO комиссию не платит | комиссия=0
  OK  без PRO — стандартная комиссия | комиссия=500
3. Метка PRO в профиле
  OK  истёкшая PRO: is_pro=False в профиле
  OK  флаг без срока: is_pro=False в профиле
  OK  действующая PRO: is_pro=True в профиле
=== ИТОГ: passed=9 failed=0 ===
```

Celery при этом остаётся необязательным: задача `check_expired_pro_subscriptions`
по-прежнему не запускается, но её отсутствие больше ничего не даёт — флаг истёкшей
подписки может оставаться `True`, прав он не предоставляет. Если воркер когда-нибудь
поднимут, задача просто начнёт дополнительно чистить флаг, что безвредно.

### Осталось (не блокирует)

- **`render.yaml` неполон как контур развёртывания.** В нём нет ни одной
  платёжной переменной (`YOOKASSA_SHOP_ID`, `YOOKASSA_SECRET_KEY`,
  `YOOMONEY_ACCESS_TOKEN`, `YOOMONEY_CLIENT_ID`, `YOOMONEY_REDIRECT_URI`,
  `YOOMONEY_NOTIFICATION_SECRET`), ни `REDIS_URL`, ни сервиса бота — бот есть
  только в `docker-compose.prod.yml:97`. При развёртывании на Render приём
  платежей не заработает, а `is_configured()` тихо вернёт `False`.
...
- **Redis в dev не поднят** → rate-limit и кэш работают в памяти процесса
  ...
  На Render он тоже не задан (см. выше).
- **Версии Python разъезжаются.** CI гоняет `"3.13"`, а `backend/Dockerfile:1`
  и `render.yaml` пиннят `3.11`. Проверено `ast.parse(..., feature_version=(3, 11))`
  по всем файлам бэкенда: пост-3.11 синтаксиса нет ни в одном, то есть сегодня
  это не ломает, но CI проверяет не тот интерпретатор, что едет в прод.
