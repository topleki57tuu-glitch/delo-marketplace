...
**8. Истёкшая PRO-подписка сохраняет все привилегии.** Подтверждено на
живом стенде.
...
Все потребители смотрят только на булев флаг и никогда — на `pro_until`:
...
Два способа починить, и они не равнозначны:

- *Дорогой:* сделать `celery` настоящей зависимостью и поднять worker + beat
  в каждом контуре развёртывания. Даёт «правильную» архитектуру, но добавляет
  сервис в три конфига и всё равно оставляет окно до 24 часов.
- *Дешёвый и надёжный:* проверять `pro_until` в момент чтения. Например,
  свойство модели `is_pro_active` → `is_pro and (pro_until is None or
  pro_until > utcnow())` и использование его вместо `is_pro` в четырёх местах
  расчёта прав. Окно исчезает вовсе, отдельный процесс не нужен.

Ни один из двух контуров развёртывания не поднимает Celery, поэтому «правильный»
вариант требует правок во всех трёх конфигах — а второй вариант не требует
ничего, кроме четырёх строк.
