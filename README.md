...
- **Арбитраж**: любая сторона сделки может открыть спор; арбитры (email в `ADMIN_EMAILS`) видят споры на `/disputes` и выносят решение — возврат заказчику или выплата исполнителю; инициатор спора может его отозвать с отменой заказа
...
Покрывает: регистрацию/вход, роли, создание заказа, отклик и кредиты, эскроу (холд, выплата, **возврат при отмене и арбитраже**), чат (REST + WebSocket broadcast), уведомления, отзывы, монетизацию, защиту от двойной выплаты, споры и решения арбитра, каталог специалистов (поиск/сортировка/пагинация), CSV-экспорт, сброс пароля.
...
- **Реализовано в этом спринте**: споры/арбитраж с возвратом эскроу (страница `/disputes`, арбитр admin@delo.ru), отмена назначения с возвратом средств, каталог специалистов `/specialists` (поиск, город, сортировка, пагинация), история операций и CSV-экспорт в профиле, загрузка портфолио через UI, флоу «Забыли пароль?» (страница `/reset`; без SMTP в dev ссылка показывается в ответе API)
...
*Последнее обновление: 2026-09-10 (вечер)*
