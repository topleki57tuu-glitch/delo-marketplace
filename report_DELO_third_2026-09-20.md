...
Дата: 20.09.2026
База: `topleki57tuu-glitch/delo-marketplace`, коммит `6f1c000` → исправлено и запушено (`a6adfcd` … `bc3287c`, десять коммитов)
Проверялось: 21 набор, живой сервер, чистый venv из объявленных зависимостей, настоящий Redis в процессе
...
Все пять исправлены и закрыты регрессами. Плюс закрыто слепое пятно в проверке документации, поправлена латентная сериализация enum и найдено несколько вещей, которые решает пользователь, а не я.
...
| Группа 2: 11 самодостаточных наборов | 11/11 |
| Группа 2 в чистом venv из `requirements-dev.txt` | 11/11 (и падение без `httpx` — воспроизведено) |
...
**Остальные 21 копия не тронуты.** Это отдельная механическая правка по всему `app/` (`auth.py`, `chat.py`, `disputes.py`, `tasks.py`, `users.py`, `verification.py`, `withdrawals.py`), и делать её молча в рамках этого захода я не стал: у полей вроде `tasks.status` или `users.role` есть значения по умолчанию, так что сейчас они всегда заполнены. Помощник для этого уже лежит в `app/core/enums.py` — замена сводится к `enum_value(...)`.
...
- **CI на последних коммитах.** Локально обе группы зелёные, но прогон GitHub Actions на `bc3287c` я не видел — он стартует после пуша. Первый прогон на `a6adfcd` был красным именно из-за D5, так что подтверждение нужно.
- **Push.** Ушёл не сразу: `git credential.helper` = `helper-selector` виснет (код 124), а прямой `git-credential-wincred` отдаёт учётку `aacopov-debug` без прав на репозиторий (403). Через несколько попыток селектор ответил и все десять коммитов ушли. Если повторится — причина в этом, а не в репозитории.
- **Остальные 21 копия выражения `hasattr(x, "value")`** в `app/` (D9) — не тронуты, помощник `enum_value` готов.
...
## Ловушка, съевшая время

Первый прогон группы 1 дал 401 на демо-аккаунтах в четырёх наборах, и это выглядело как дефект авторизации. Причина оказалась в окружении: **на порту 8000 висел сервер из прошлой сессии**, со старой базой и старым паролем. Мой `uvicorn` не поднялся — в логе `[Errno 10048] error while attempting to bind`, — а тесты всё это время ходили в чужой процесс. Проверка «сервер отвечает на `/csrf-token`» это не ловит: чужой сервер отвечает тоже.
...
| `.github/workflows/ci.yml` | три новых набора в группе 2; джоба `backend` ставит dev-зависимости |

Коммиты: `528dc75`, `d472ae1`, `a6adfcd`, `e034aa8`, `4356e05`, `8ab6f47`, `413a474`, `f9ad01c`, `bf7a582`, `bc3287c` — все на GitHub.
