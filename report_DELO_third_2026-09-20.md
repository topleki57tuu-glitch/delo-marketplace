...
Дата: 20.09.2026
База: `topleki57tuu-glitch/delo-marketplace`, коммит `6f1c000` → исправлено (`a6adfcd` + четыре локальных)
Проверялось: 20 наборов, живой сервер, чистый venv из объявленных зависимостей, настоящий Redis в процессе
...
Все четыре исправлены, все закрыты регрессами. Плюс закрыто слепое пятно в проверке документации и найдено несколько вещей, которые решает пользователь, а не я.
...
| Группа 2: 10 самодостаточных наборов | 10/10 |
| Группа 2 в чистом venv из `requirements-dev.txt` | 10/10 (и падение без `httpx` — воспроизведено) |
...
Отдельно названные числа: `e2e_api_test` 48, `test_csrf_coverage` 15, `test_yoomoney_webhook` 17, `check_file_cleanup` 18, `repro_verification_document_access` 24, `check_online_status` 17, `check_tasks_list_cache` 14 (новый).
...
- **CI на новых коммитах.** Запушить не удалось: `git credential.helper` в этом окружении — `helper-selector`, и он не отвечает (виснет, 240 с, код 124). Напрямую `git-credential-wincred` отвечает, но лежащая там учётка (`aacopov-debug`) не имеет прав на репозиторий: `403 Permission denied`. Коммиты `e034aa8`, `4356e05` и текущий лежат локально в `main` и ждут `git push`. Первые три коммита (`528dc75`, `d472ae1`, `a6adfcd`) ушли нормально, пока селектор отвечал.
...
| `.github/workflows/ci.yml` | два новых набора в группе 2; джоба `backend` ставит dev-зависимости |

Коммиты: `528dc75`, `d472ae1`, `a6adfcd` — на GitHub. `e034aa8`, `4356e05`, `8ab6f47`, `413a474` — локально, ждут `git push` (см. «Что осталось непроверенным»).
