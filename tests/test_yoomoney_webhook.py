...
def main():
    ...
    #
    # Здесь проверяется только направление «прислано БОЛЬШЕ». Обратное —
    # «уплачено меньше запрошенного» — было настоящей дырой (оплатить 100 ₽
    # по счёту на 100 000 ₽), и его проверяет отдельный набор:
    # tests/security/check_payment_amount_credit.py. Правило целиком —
    # «минимум из запрошенного и уплаченного», а не «всегда наша записанная».
