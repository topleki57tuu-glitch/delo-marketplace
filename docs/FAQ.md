...
python set_password.py user@example.com --no-save          # не трогать demo_password.txt
```
...
иначе файл начинает врать, а именно из него пароль берут и люди, и скрипты.
Файл, которого нет, скрипт не создаёт: на боевом контуре его быть не должно.
