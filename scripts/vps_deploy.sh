...
# Гарантируем перевод строки в конце .env перед любым `>>`. Иначе новый ключ
# приклеивается к последней существующей строке, и обе переменные становятся
# битыми — молча, без ошибок в логах (уже случалось: RETURN_PATH слипся с
# REDIRECT_URI и получился https:///...).
...
echo " Апдейт: bash scripts/vps_update.sh   (подтянет код, накатит миграции,"
echo "         пересоберёт — .env не трогает)"
echo "=========================================="
